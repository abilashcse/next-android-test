...
import co.uk.next.techtest.data.local.MIGRATION_1_2
import co.uk.next.techtest.data.local.TechTestDatabase
import co.uk.next.techtest.data.remote.DummyJsonApiClient
import co.uk.next.techtest.data.repository.BagProductsRepositoryImpl
...
import co.uk.next.techtest.presentation.bag.BagViewModel
...
val appModule = module {
    ...
    single { get<TechTestDatabase>().bagProductDao() }
    single<SavedProductsRepository> { SavedProductsRepositoryImpl(dao = get()) }
    single<BagProductsRepository> { BagProductsRepositoryImpl(dao = get()) }
    ...
    viewModel {
        ProductDetailsViewModel(
            ...
            savedProductsRepository = get(),
            bagProductsRepository = get()
        )
    }
    ...
    viewModel { BagViewModel(bagProductsRepository = get(), savedProductsRepository = get()) }
}
